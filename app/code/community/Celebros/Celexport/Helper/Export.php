...
class Celebros_Celexport_Helper_Export extends Mage_Core_Helper_Abstract
{
    ...
    public function getCalculatedPrice($product)
    {
        ...
        if ($product->getData("type_id") == "bundle") {
            ...
            $storeTimestamp = Mage::app()->getLocale()->storeTimeStamp($storeId);
            $priceRules = Mage::getResourceModel('catalogrule/rule')->getRulePrice($storeTimestamp, $websiteId, null, $product->getEntityId());
            $price = ($priceRules < $price && $priceRules > 0) ? $priceRules : $price;
        } else {
            $price = $product->getFinalPrice();
            $priceRules = Mage::getModel('catalogrule/rule')->calcProductPriceRule($product, $product->getFinalPrice());
            $price = ($priceRules < $price && $priceRules > 0) ? $priceRules : $price;
        }
       
        switch (Mage::helper('celexport')->getCurrency()) {
            case 'base_with':
                $result = Mage::helper('tax')->getPrice($product, $price);
                break;
            case 'default_without':
                $result = Mage::app()->getStore()->convertPrice($price);
                break;   
            case 'default_with':
                $result = Mage::helper('tax')->getPrice(
                    $product,
                    Mage::app()->getStore()->convertPrice($price)
                );
                break;
            default:
                $result = $price;
    ...
    public function getProductsData($ids, $customAttributes, $storeId)
    {
        ...
        foreach ($collection as $product) {
            ...
            $fEnclose = Mage::getStoreConfig('celexport/export_settings/enclosed_values');
            if ($fDel === '\t') $fDel = chr(9);
            
            $str.= $fEnclose . implode($fEnclose . $fDel . $fEnclose, $values) . $fEnclose . "\r\n";
