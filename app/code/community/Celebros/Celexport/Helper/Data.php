...
class Celebros_Celexport_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function getFtpEnvStamp($store = NULL)
    {
        return Mage::getStoreConfig('celexport/export_settings/env_stamp', $store);
    }
    
    public function getCurrentEnvStamp()
    {
        /*return base64_encode(Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB));*/
        $stamp = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB);
        $stamp = str_replace(array('http://', 'https://'), '', $stamp);
        return $stamp;
    }

    public function getCurrency($store = NULL)
    {
        return Mage::getStoreConfig('celexport/export_settings/currency', $store);
    }    
}
