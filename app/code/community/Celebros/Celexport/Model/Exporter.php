...
class Celebros_Celexport_Model_Exporter
{
     ...
     * This method is called from cron process, cron is workink in UTC time
     *
     * @param Mage_Cron_Model_Schedule $schedule
     */
    public function catalogUpdate(Mage_Cron_Model_Schedule $schedule)
    {
    ...
    public function export_celebros($webAdmin)
    {
        ...
        if (!$model->export_celebros($webAdmin, $this->_exportProcessId)) {
            return false;
        }
    ...
    public function comments_style($kind, $text, $alt)
    {
        ...
        switch ($kind) {
            ...
            break;
            case 'icon':
                $html = '<li style="background-color: rgb(128, 128, 128); color:rgb(255,255,255);">
                <img style="margin-right: 5px;" src="' . Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_SKIN) . '/adminhtml/default/default/images/note_msg_icon.gif" alt='.$alt.'/>
                '.$text.'
                </li>';
            break;
            case 'info':
                ...
                break;
            case 'warning':
                $html = '<li style="background-color: rgb(255, 255, 128);">
                <img style="margin-right: 5px;" src="' . Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_SKIN) . '/adminhtml/default/default/images/fam_bullet_error.gif" alt='.$alt.'/>
                '.$text.'
                </li>';
            break;
            case 'success':
                $html = '<li style="background-color: rgb(128, 255, 128);">
                <img src="' . Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_SKIN) . '/adminhtml/default/default/images/fam_bullet_success.gif" alt='.$alt.'/>
                '.$text.'
                </li>';
            break;
            case 'section':
                $html = '<li style="background-color: rgb(100, 149, 237);">
                <img src="' . Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_SKIN) . '/adminhtml/default/default/images/fam_bullet_success.gif" alt='.$alt.'/>
                '.$text.'
                </li>';
            break;
            case 'error':
                $html = '<li style="background-color: rgb(255, 187, 187);">
                <img src="' . Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_SKIN) . '/adminhtml/default/default/images/error_msg_icon.gif" alt='.$alt.'/>
                '.$text.'
                </li>';
            break;
            default:
    ...
    public function export_config($store_id)
    {
        ...
        if (Mage::app()->getRequest()->getParam("upload") == "false") {
            $this->_bUpload = FALSE;
        ...
        if ($currentEnvStamp == $this->_fStore->getConfig('celexport/ftp_prod/env_stamp')) {
            $ftppath = 'ftp_prod';
        } elseif ($currentEnvStamp == $this->_fStore->getConfig('celexport/ftp_dev/env_stamp')) {
            $ftppath = 'ftp_dev';
        } else {
            $ftppath = NULL;
        }
        
        $this->_fFTPHost = $ftppath ? $this->_fStore->getConfig('celexport/' . $ftppath . '/ftp_host') : NULL;
        $this->_fFTPPort = $ftppath ? $this->_fStore->getConfig('celexport/' . $ftppath . '/ftp_port') : NULL;
        $this->_fFTPUser = $ftppath ? $this->_fStore->getConfig('celexport/' . $ftppath . '/ftp_user') : NULL;
        $this->_fFTPPassword = $ftppath ? $this->_fStore->getConfig('celexport/' . $ftppath . '/ftp_password') : NULL;
        $this->_fFTPPassive = $ftppath ? $this->_fStore->getConfig('celexport/' . $ftppath . '/passive') : NULL;*/
    ...
    public function export_orders_celebros()
    {
        ...
        foreach ($store_view_ids as $store_view_id) {
            ...
            || !Mage::app()->getStore($store_view_id['store_id'])->getConfig('celexport/export_settings/export_data_history')) {
                continue;
            }
            ...
            while ($row = $stm->fetch()) {
                ...
                $record["ProductID"] = $row["sku"]; //$row["product_id"];
    ...
    protected function _getOrdersSql()
    {
        ...
        if ($this->_isStoreIdColumnExist($orderItemTable)) {
            $sql = "SELECT order_id, sku, product_id, created_at, qty_ordered, row_total, row_total_incl_tax
            FROM {$orderItemTable}
    ...
    public function scheduleNewExport($observer = NULL)
    {
        ...
        if ($config instanceof Mage_Core_Model_Config_Element) {
            ...
            foreach ($jobs as $jobCode => $jobConfig) {
                ...
                try {
                    ...
                    ->save();
                    $this->addToBody("{$jobCode} cron job is scheduled at $timescheduled <br/>");
