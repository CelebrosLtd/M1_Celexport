...
class Celebros_Celexport_Model_ObserverLarge extends Celebros_Celexport_Model_Exporter
{
    ...
    public function exportProdIdsByAttributeValue($attribute_code, $attribute_value, $store_id = 0, $filename)
    {
        ...
        $str = $this->_fEnclose . "entity_id" . $this->_fEnclose . "\r\n";
        
        foreach ($collection as $item) {
            $str .= $this->_fEnclose . $item->getEntityId() . $this->_fEnclose . "\r\n";
        }
    ...
    protected function export_table_rows($sql, $fields, $fh)
    {
        ...
        while ($row = $query->fetch()) {
            ...
            $str .= $this->_fEnclose . implode($this->_fEnclose . $this->_fDel . $this->_fEnclose, $row) . $this->_fEnclose . "\r\n";
            $rowCount++;
    ...
    {   
        ...
        $header .= implode($this->_fEnclose . $this->_fDel . $this->_fEnclose, $fields);
        $header .= $this->_fEnclose . "\r\n";
    ...
    protected function export_products($sql, $fileName)
    {
        ...
        $header = $this->_fEnclose . implode($this->_fEnclose . $this->_fDel . $this->_fEnclose, $fields) . $this->_fEnclose . "\r\n";
        $this->write_to_file($header, $fh);
